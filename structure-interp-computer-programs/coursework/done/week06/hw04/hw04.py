...
def balanced(m):
    ...
    False
    """
    # TORQUE == arm_length * weight_on_arm

    if is_planet(m):
        return True
    Ltorq = length(left(m)) * total_weight(end(left(m)))
    Rtorq = length(right(m)) * total_weight(end(right(m)))
    Lbalanced = balanced(end(left(m)))
    Rbalanced = balanced(end(right(m)))
    return True if (Ltorq == Rtorq and Lbalanced and Rbalanced) else False
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    # fail on init
    acct = withdraw(0, old_pass)
    if type(acct) != int:
        return acct  # if acct is not a number, then it is a str : fail message

    # init has succeeded
    def joint_wrapper(amount, passwd):

        if passwd == new_pass or passwd == old_pass:
            return withdraw(amount, old_pass)  # either passwd works
        return withdraw(amount, passwd)  # neither passwd was correct; pass in
        # failing passwd to register the failure
