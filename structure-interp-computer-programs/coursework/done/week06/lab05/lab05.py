...
def couple(lst1, lst2):
    ...
    return [list(x) for x in list(zip(lst1, lst2))]
...
def distance(city1, city2):
    ...
    latB = get_lat(city2)
    lonB = get_lon(city2)
    return sqrt((latB - latA) ** 2 + (lonB - lonA) ** 2)
...
def closer_city(lat, lon, city1, city2):
    ...
    geometry = {get_name(c) : distance(loc, c) for c in [city1, city2]}
    return min(geometry, key=geometry.get)
...
def sprout_leaves(t, values):
    ...
    """
    if is_leaf(t):
        return tree(t[0], [tree(v) for v in values])
    for branch in t[1:]:
        return tree(t[0], [sprout_leaves(branch, values) for branch in t[1:]])
...
def add_trees(t1, t2):
    ...
    x = lt1 if min(len(lt1),len(lt2)) == len(lt1) else lt2
    y = lt2 if x == lt1 else lt1
    x += [''] * abs(len(lt1) - len(lt2))
    
    # I had help with this one...
    return tree(label(t1) + label(t2), [add_trees(b1,b2) for b1,b2 in zip( lt1, lt2 )])
